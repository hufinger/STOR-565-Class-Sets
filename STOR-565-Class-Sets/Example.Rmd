---
title: "Classification"
author: "James D. Wilson"
date: "March 23rd, 2018"
output:
  html_document: default
---

```{r}
library(readr)
library(tidyverse)
library(caret)
library(caretEnsemble)
library(dplyr)
library(e1071)
library(aod)
```

```{r}
setwd('~/STOR-565-Class-Sets')

set.seed(123)

combine.college <- read_csv("NFL-CFB-Combine.csv")


combine.college$combine_Height = gsub('Jul', '7ft', fixed = T, combine.college$combine_Height)



combine.college$combine_Height = gsub('Jun', '6ft', fixed = T, combine.college$combine_Height)



combine.college$combine_Height = gsub('May', '5ft', fixed = T, combine.college$combine_Height)



combine.college$combine_Height = gsub('Apr', '5ft', fixed = T, combine.college$combine_Height)





feet = ifelse(grepl('6ft', combine.college$combine_Height),
              
              6,
              
              5)

inches = combine.college$combine_Height

inches = gsub('6ft', '',inches)

inches = gsub('5ft', '',inches)



inches = gsub('-', '',inches, fixed = T)

inches = as.numeric(inches)

combine.college$combine_Height = feet*12 + inches


combine.included = filter(combine.college, !is.na(combine_Year)) %>% filter(!is.na(cfb_School))



combine.included = 
  combine.included %>% dplyr::select(-cfb_Att,
                                     -cfb_ReceivingAvg)


cfb.crap = combine.included[,which(grepl('cfb_', colnames(combine.included)))]

cfb.crap = cfb.crap[,4:ncol(cfb.crap)]


cfb.means = data.frame(matrix(ncol = 12, nrow = nrow(cfb.crap)))

cfb.sums = data.frame(matrix(ncol = 12, nrow = nrow(cfb.crap)))

colnames(cfb.means) = gsub('Senior', 'Mean_', 
                           colnames(cfb.crap)[seq(from = 1, by= 5, to = ncol(cfb.crap))])

colnames(cfb.sums) = gsub('Senior', 'Sum_', 
                          colnames(cfb.crap)[seq(from = 1, by= 5, to = ncol(cfb.crap))])

my.mean = function(x){
  
  return(mean(x, na.rm = T))
}


my.sum = function(x){
  
  return(sum(x, na.rm = T))
}

for(i in 1:12){
  
  cfb.means[,i] = apply(X = cfb.crap[,i:c(i+4)], MARGIN = 1, FUN = my.mean)
  
  cfb.sums[,i] = apply(X = cfb.crap[,i:c(i+4)], MARGIN = 1, FUN = my.sum)
  
  
}


combine.included = cbind(combine.included,
                         cfb.means,
                         cfb.sums)


summary = combine.included %>% group_by(combine_Pos) %>% summarise(freq = n())


#remove qb's and fullbacks since we had 24 and 1 respectively

combine.included = combine.included %>% dplyr::filter(combine_Pos != 'QB' & combine_Pos != 'FB')

combine.included$`nfl_Rushing Yds` = ifelse(is.na(combine.included$`nfl_Rushing Yds`),
                                            0, combine.included$`nfl_Rushing Yds`)

combine.included$`nfl_Yds` = ifelse(is.na(combine.included$`nfl_Yds`),
                                    0, combine.included$`nfl_Yds`)


#PREDICTING ON SCRIMMAGE YARDS PER GAME
combine.included$scrimmageYds = 
  (combine.included$`nfl_Rushing Yds` + combine.included$nfl_Yds)


model.data = dplyr::select(combine.included, -c(contains("nfl"), combine_College, contains("combine_Drafted")))


#testing with WR (dominant position)

flex.data = model.data %>% dplyr::filter(combine_Pos  %in% c('WR', 'RB', 'TE'))


#only make college data 0s, as we don't want 0 bench press reps (it needs to stay NA)
flex.data = flex.data %>% mutate_at(.vars = which(grepl('cfb_',
                                                        colnames(flex.data))), 
                                    .funs = funs(ifelse(is.na(.), 0, .)))



flex.data = flex.data%>% dplyr::select(-combine_School,
                                       -combine_Year,
                                       -combine_AV)

na.counts.flex.data = colSums(is.na(flex.data))


#only grab players with complete combine data
flex.data = flex.data[complete.cases(flex.data),]


flex.data = flex.data[,4:ncol(flex.data)] 

player.conf = flex.data$cfb_Conf

#need to remove due to interaction issues
flex.data = flex.data %>% dplyr::select(-cfb_Conf)


flex.data = flex.data %>% dplyr::select(scrimmageYds,
                                        everything())


nums <- unlist(lapply(flex.data, is.numeric))  

numeric.data = flex.data[,nums]

library(mctest)
imcdiag(numeric.data, y = numeric.data$scrimmageYds,
        method = 'VIF')




# flex.data$combine_Pos = as.factor(flex.data$combine_Pos)


#creating dummy variables
flex.data$is.catcher = ifelse(flex.data$combine_Pos %in% c('WR',
                                                           'TE'),
                              1, 0)


flex.data$is.wr = ifelse(flex.data$combine_Pos %in% c('WR'),
                         1, 0)
flex.data$is.rb = ifelse(flex.data$combine_Pos == 'RB',
                         1, 0)

flex.data$is.te = ifelse(flex.data$combine_Pos == 'TE',
                         1, 0)

#have dummy variables so can now get rid of factored position variable
flex.data = flex.data %>% dplyr::select(-combine_Pos)


# only have enough obs for power five, otherwise just make it other
player.conf = ifelse(player.conf %in% 
                       c('American',
                         'Big East',
                         'CUSA',
                         'Ind',
                         'MAC',
                         'MWC',
                         'Sun Belt'),
                     'Non Power 5',
                     player.conf)

flex.data$cfb_Conf = as.factor(player.conf)

bad.columns = flex.data[, nearZeroVar(flex.data)]


#got rid of freshman, rs freshman seasons as that was most of the near zero var issue

#very few players had 4 our more seasons of data, and it did not improve predictions anyway

flex.data  = flex.data[,-c(which(grepl('_Freshman', colnames(flex.data))))]

#below shows there are now zero near zero var columns
bad.columns = flex.data[, nearZeroVar(flex.data)]


rb.data = flex.data %>% filter(is.rb == 1) %>% dplyr::select(-is.wr,
                                                      -is.te,
                                                      -is.catcher,
                                                      -is.rb)

bad.columns = rb.data[, nearZeroVar(rb.data)]


catcher.data = flex.data %>% filter(is.catcher == 1) %>% dplyr::select(
  -is.catcher,
  -is.rb)
bad.columns = catcher.data[, nearZeroVar(catcher.data)]





flex.data.classifier = flex.data

for(i in 1:nrow(flex.data.classifier)){
  if(flex.data.classifier$scrimmageYds[i] < 480){
    flex.data.classifier$success[i] = 0
  }else{
    flex.data.classifier$success[i] = 1
  }
}


rb.data.classifier = rb.data

for(i in 1:nrow(rb.data)){
  if(rb.data.classifier$scrimmageYds[i] < 480){
    rb.data.classifier$success[i] = 0
  }else{
    rb.data.classifier$success[i] = 1
  }
}

catcher.data.classifier = catcher.data

for(i in 1:nrow(catcher.data)){
  if(catcher.data.classifier$scrimmageYds[i] < 480){
    catcher.data.classifier$success[i] = 0
  }else{
    catcher.data.classifier$success[i] = 1
  }
}



catcher.data.classifier = dplyr::select(catcher.data.classifier, -c(contains("conf"), contains("is."), contains("rushing"), contains("Plays"), contains("sum"), scrimmageYds))
rb.data.classifier = dplyr::select(rb.data.classifier, -c(contains("conf"), contains("is."), contains("Plays"), contains("sum"), scrimmageYds))

catcher.data.classifier$success = as.factor(catcher.data.classifier$success)
rb.data.classifier$success = as.factor(rb.data.classifier$success)

train = sample(1:dim(catcher.data.classifier)[1], dim(catcher.data.classifier)[1]/100*66)
catcher.train = catcher.data.classifier[train,]
catcher.test = catcher.data.classifier[-train,]

logistic.rb = glm(success~., data = rb.data.classifier, family = "binomial")
logistic.catcher = glm(success~., data = catcher.train, family = "binomial")
summary(logistic.rb)
summary(logistic.catcher)

train = sample(1:dim(rb.data.classifier)[1], dim(rb.data.classifier)[1]/100*66)
rb.train = rb.data.classifier[train,]
rb.test = rb.data.classifier[-train,]

library(MASS)
library(class)

lda.mod = lda(success ~ ., data = catcher.train)
lda.mod

lda.rb = lda(success ~ ., data = rb.train)
lda.rb

cost_out <- seq(from = 0.1, to =5.1, by = 1)
missed = rep(NA, length(cost_out))
for(i in 1:length(cost_out)){
  svm.mod = svm(formula = success~., data =catcher.train, kernel = "linear", cost = cost_out[i], fold = 5)
  missed[i] = (sum(svm.mod$fitted != catcher.train$success))/nrow(catcher.train)
}
svm.mod = svm(formula = success~., data =catcher.train, kernel = "linear", cost = 3.1, fold = 5)
svm.rad = svm(formula = success~., data =catcher.train, kernel = "radial", cost = 3.1, fold = 5)

svm.pred = predict(svm.mod, newdata = catcher.test)
confusionMatrix(svm.pred, catcher.test$success)

svm.pred = predict(svm.rad, newdata = catcher.test)
confusionMatrix(svm.pred, catcher.test$success)

lda.pred = predict(lda.mod, newdata = catcher.test)
lda.class = lda.pred$class
table(lda.class, catcher.test$success)

missed = rep(NA, length(cost_out))
for(i in 1:length(cost_out)){
  svm.mod = svm(formula = success~., data =rb.train, kernel = "linear", cost = cost_out[i], fold = 5)
  missed[i] = (sum(svm.mod$fitted != rb.train$success))/nrow(rb.train)
}
svm.mod = svm(formula = success~., data =rb.train, kernel = "linear", cost = 1.1, fold = 5)
svm.rad = svm(formula = success~., data =rb.train, kernel = "radial", cost = 1.1, fold = 5)


svm.rb = predict(svm.mod, newdata = rb.test)
confusionMatrix(svm.rb, rb.test$success)

svm.rb.rad = predict(svm.rad, newdata = rb.test)
confusionMatrix(svm.rb.rad, rb.test$success)

lda.rb = predict(lda.rb, newdata = rb.test)
lda.class = lda.rb$class
table(lda.class, rb.test$success)

confusion <- function(yhat, y, quietly = FALSE){
  
  if(!quietly)
    message("yhat is the vector of predicted outcomes, possibly a factor.\n
          Sensitivity = (first level predicted) / (first level actual) \n
          Specificity = (second level predicted) / (second level actual)")
  
  if(!is.factor(y) & is.factor(yhat))
    y <- as.factor(y)
  
  if(!all.equal(levels(yhat), levels(y)))
    stop("Factor levels of yhat and y do not match.")
  
  confusion_mat <- table(yhat, y, deparse.level = 2)
  stats <- data.frame(sensitivity = confusion_mat[1, 1]/sum(confusion_mat[, 1]),
                                 specificity = confusion_mat[2, 2]/sum(confusion_mat[, 2]))
  
  return(list(confusion_mat = confusion_mat, stats = stats))
}

knn_cv = function(data){
  numrow = (nrow(data)/4)
  data = data[sample(nrow(data)),]  
  folds = cut(seq(1,nrow(data)), breaks = 4, labels = F)
  knn_model <- list()
  for(i in 1:4){
    testIDs = which(folds == i, arr.ind = T)
    test = data[testIDs,]
    ktest = dplyr::select(test, -success)
    train = data[-testIDs,]
    ktrain = dplyr::select(train, -success)
    knn_model[[i]] <- knn(ktrain, ktest, cl = train$success, k = i)
    
    
  }
  
knn_results <- lapply(knn_model, FUN = function(x){
  return(confusion(x, test$success, quietly = TRUE)$stats)
  })

knn_results <- bind_rows(knn_results)
knn_results$K <- 1:4
error = matrix(rep(NA, 1, 4))
for(i in 1:4){
testtab = table(knn_model[[i]], test$success)
  error[i] = (testtab["0", "1"] + testtab["1", "0"])/numrow
}
print(error)
}
knn_cv(catcher.data.classifier[1:80,])
knn_cv(rb.data.classifier)
```


